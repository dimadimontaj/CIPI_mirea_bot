...
from keyboards import get_jokes_keyboard, get_start_keyboard
...
async def get_start(message: Message, bot: Bot):
    await bot.send_photo(
        ...
        caption=emoji.emojize(
            "Привет! Сегодня ты узнаешь много "
...
async def get_help(message: Message, bot: Bot):
    msg = await bot.send_message(
        chat_id=message.from_user.id,
        text=emoji.emojize("никакой помощи нет :middle_finger:"),
    )
    await message.delete()
...
async def get_jokes(call: CallbackQuery, callback_data: CategoryInfo):
    ...
    try:
        ...
        await call.message.edit_caption(
            caption=f"{joke.jokes} \n\n\nРейтинг анекдота: {joke.rating}",
            reply_markup=get_jokes_keyboard(category, joke.joke_id),
        )
    except Exception:
        pass
    await call.answer("")
...
async def change_rating(call: CallbackQuery, callback_data: RatingInfo):
    ...
    await call.message.edit_caption(
        caption=f"{joke.jokes} \n\n\nРейтинг анекдота: {joke.rating}",
        reply_markup=get_jokes_keyboard(joke.category, joke.joke_id),
    )
